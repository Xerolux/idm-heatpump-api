...
prerelease section of that version is folded into the single ``## [<version>]``
section. This repository spells prereleases in PEP 440 (``2.2.0b1``,
``2.2.0rc1``); SemVer spellings (``2.2.0-beta.1``) are accepted all the same.
Individual prereleases are not listed separately after the cut, but nothing
that changed may be dropped by the fold. The full per-prerelease history
stays in git and in the GitHub releases of the prerelease tags.
...
Usage::
    ...
    python scripts/consolidate_changelog.py --version 2.2.0 [--date 2026-10-01]
...
def fold(text: str, version: str, date: str) -> str:
    ...
    pre = [
        ...
        if _prerelease_version(heading) == version
    ]
    if any(
    ...
    for intro in intros:
        lines.extend([intro, ""])
    ordered = [
        (category, bullets[category]) for category in CATEGORY_ORDER if bullets.get(category)
    ]
    ordered += sorted(
        (category, items)
        for category, items in bullets.items()
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--date", default="UNRELEASED DRAFT", help="release date, or the draft marker"
    )
