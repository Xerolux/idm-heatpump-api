...
## Optional numeric step (2.2.0)

`RegisterDef.step` and registry schema `step` expose an optional physical-unit
increment for consumer controls. Existing constructors remain compatible.
Unknown steps and ranges stay `None`; the API does not infer bounds, quantize
writes, or alter Modbus requests. See [Register Metadata](Register-Metadata.md)
for the source and scope of the heating-curve and zone-setpoint hints.
