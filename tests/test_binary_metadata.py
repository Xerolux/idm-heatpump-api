...
def _metadata(name: str) -> BinaryRegisterMetadata:
    metadata = get_binary_register_metadata(name)
    assert metadata is not None
    return metadata


def test_core_binary_metadata_is_available() -> None:
    assert _metadata("heating_demand").device_class == "heat"
    assert _metadata("cooling_demand").device_class == "cold"
    assert _metadata("dhw_demand").device_class == "heat"
    assert _metadata("hp_sum_alarm").device_class == "problem"
    assert _metadata("compressor_status_1").device_class == "running"


def test_zone_room_relays_use_pattern_metadata() -> None:
    metadata = _metadata("zm3_room6_relay")
...
def test_explicit_metadata_points_to_existing_binary_registers() -> None:
    registers = build_register_map(circuits=[], zone_modules=0)
